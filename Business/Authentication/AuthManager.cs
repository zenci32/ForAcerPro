...
namespace Business.Authentication
{
    public class AuthManager : IAuthService
    {
        ...
        public async Task<IDataResult<Token>> Login(LoginAuthDto loginDto)
        {
            ...
            if (user == null)
                return new ErrorDataResult<Token>(null, "Kullanıcı maili sistemde bulunamadı!",404);

            if (!user.IsConfirm)
               return new ErrorDataResult<Token>(null,"Kullanıcı maili onaylanmamış!",404);
            ...
            return new ErrorDataResult<Token>(null,"Kullanıcı maili ya da şifre yanlış",404);
        }
        ...
        private async Task<IResult> CheckIfEmailExists(string email)
        {
            ...
            if (list != null)
            {
                return new ErrorResult("Bu mail adresi daha önce kullanılmış",404);
            }
